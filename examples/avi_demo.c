...
{
    printf("usage: %s <input.avi> <output_dir> [packet_count]\n", program);
}

static void print_stream_info(const avi_demux_t *demuxer)
{
    char handler[5];
    char compression[5];

    printf("AVI File:\n");
    printf("  file_size  : %u\n", (unsigned int)avi_get_file_size(demuxer));
    printf("  streams    : %u\n", (unsigned int)avi_get_stream_count(demuxer));
    printf("  index_count: %u\n", (unsigned int)avi_get_index_count(demuxer));
    if (demuxer->has_video != 0u) {
        avp_fourcc_to_string(demuxer->video.stream.fccHandler, handler);
        avp_fourcc_to_string(demuxer->video.format.biCompression, compression);
        printf("Video Stream:\n");
        printf("  stream_index: %u\n", (unsigned int)demuxer->video.stream_index);
        printf("  codec       : %s (handler=%s, compression=%s)\n",
               avi_video_codec_name(demuxer->video.codec_type),
               handler,
               compression);
        printf("  size        : %ux%u\n",
               (unsigned int)avi_get_width(demuxer),
               (unsigned int)avi_get_height(demuxer));
        printf("  samples     : %u\n", (unsigned int)demuxer->video.stream.dwLength);
    }
    if (demuxer->has_audio != 0u) {
        printf("Audio Stream:\n");
        printf("  stream_index: %u\n", (unsigned int)demuxer->audio.stream_index);
        if (demuxer->audio.format.wSubFormatTag != 0u) {
            printf("  codec       : %s (tag=0x%04x, subtype=0x%04x)\n",
                   avi_audio_codec_name(demuxer->audio.codec_type),
                   (unsigned int)demuxer->audio.format.wFormatTag,
                   (unsigned int)demuxer->audio.format.wSubFormatTag);
        } else {
            printf("  codec       : %s (tag=0x%04x)\n",
                   avi_audio_codec_name(demuxer->audio.codec_type),
                   (unsigned int)demuxer->audio.format.wFormatTag);
        }
        printf("  sample_rate : %u Hz\n",
               (unsigned int)demuxer->audio.format.nSamplesPerSec);
        printf("  channels    : %u\n", (unsigned int)demuxer->audio.format.nChannels);
        printf("  bits        : %u\n", (unsigned int)demuxer->audio.format.wBitsPerSample);
    }
}
                    ...
                    uint32_t packet_limit)
{
    ...
    st = avi_demux_open(&demuxer, &avp_io);
    if (st != AVP_OK) {
        FAIL("open AVI failed: %d\n", (int)st);
    }
    ...
    if (demuxer.has_audio != 0u) {
        ...
        st = avi_demux_get_audio_stream_config(&demuxer, &audio_config);
        if (st != AVP_OK && st != AVP_EUNSUPPORTED) {
            FAIL("get AVI audio config failed: %d\n", (int)st);
        }
        if (st == AVP_OK) {
    ...
    avi_demux_rewind(&demuxer);
    while (packet_limit == 0u || packet_count < packet_limit) {
        avp_packet_t packet;
        uint8_t *new_buffer;

        st = avi_demux_peek_packet(&demuxer, &packet);
        if (st == AVP_ENOENT) {
            break;
        }
        if (st != AVP_OK) {
            FAIL("peek AVI packet failed: %d\n", (int)st);
        }
        ...
        st = avi_demux_pop_packet(&demuxer, &packet);
        if (st != AVP_OK) {
            FAIL("pop AVI packet failed: %d\n", (int)st);
        }

        if (packet.type == AVP_PACKET_TYPE_VIDEO) {
            if (demuxer.video.codec_type == AVI_VIDEO_CODEC_MJPEG &&
