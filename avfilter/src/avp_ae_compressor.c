...
{
    return c != NULL && c->sample_rate != 0u && c->channels > 0u &&
           c->threshold_db >= -100.0f && c->threshold_db <= 0.0f && c->ratio >= 1.0f &&
           c->attack_ms > 0.0f && c->attack_ms <= 1000.0f && c->release_ms > 0.0f && c->release_ms <= 1000.0f &&
           c->makeup_db >= -10.0f && c->makeup_db <= 10.0f && (c->enable == 0u || c->enable == 1u);
}
                                       ...
                                       void *arg)
{
    ...
    switch (cmd) {
        ...
        case AVP_AE_COMPRESSOR_CMD_SET_MAKEUP_DB:
            ...
            if (!isfinite(*value))
                return AVP_ERANGE;
            if (cmd == AVP_AE_COMPRESSOR_CMD_SET_THRESHOLD_DB && (*value < -100.0f || *value > 0.0f))
                return AVP_ERANGE;
            if (cmd == AVP_AE_COMPRESSOR_CMD_SET_RATIO && (*value < 1.0f))
                return AVP_ERANGE;
            if (cmd == AVP_AE_COMPRESSOR_CMD_SET_ATTACK_MS && (*value <= 0.0f || *value > 1000.0f))
                return AVP_ERANGE;
            if (cmd == AVP_AE_COMPRESSOR_CMD_SET_RELEASE_MS && (*value <= 0.0f || *value > 1000.0f))
                return AVP_ERANGE;
            if (cmd == AVP_AE_COMPRESSOR_CMD_SET_MAKEUP_DB && (*value < -10.0f || *value > 10.0f))
                return AVP_ERANGE;
