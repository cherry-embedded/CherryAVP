...
static avp_status_t avp_ae_reverb_validate(const avp_ae_reverb_config_t *config)
{
    if (config == NULL || config->sample_rate == 0u || config->channels == 0u ||
        config->channels > 2u || !avp_ae_reverb_valid(config->room_size) ||
        !avp_ae_reverb_valid(config->damping) || !avp_ae_reverb_valid(config->wet) ||
        !avp_ae_reverb_valid(config->dry) || (config->enable != 0u && config->enable != 1u)) {
        return AVP_EINVAL;
    }
    return AVP_OK;
}
...
static void avp_ae_reverb_reset(avp_ae_reverb_t *ctx)
{
    ...
    for (i = 0u; i < AVP_AE_REVERB_DELAY_LINES; i++) {
        ...
        memset(ctx->lines[i].filter, 0, sizeof(ctx->lines[i].filter));
    }
}
