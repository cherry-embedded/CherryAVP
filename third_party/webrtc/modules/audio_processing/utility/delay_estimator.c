...
void WebRtc_FreeBinaryDelayEstimatorFarend(BinaryDelayEstimatorFarend* self) {
  ...
  avp_free(self->binary_far_history);
  self->binary_far_history = NULL;

  avp_free(self->far_bit_counts);
...
BinaryDelayEstimatorFarend* WebRtc_CreateBinaryDelayEstimatorFarend(
  ...
  if (history_size > 1) {
    ...
    self = avp_malloc(sizeof(BinaryDelayEstimatorFarend));
  }
  if (self == NULL) {
    return NULL;
  }
...
int WebRtc_AllocateFarendBufferMemory(BinaryDelayEstimatorFarend* self,
  ...
  self->binary_far_history =
      avp_realloc(self->binary_far_history,
              history_size * sizeof(*self->binary_far_history));
  self->far_bit_counts = avp_realloc(self->far_bit_counts,
...
void WebRtc_FreeBinaryDelayEstimator(BinaryDelayEstimator* self) {
  ...
  avp_free(self->mean_bit_counts);
  self->mean_bit_counts = NULL;

  avp_free(self->bit_counts);
  self->bit_counts = NULL;

  avp_free(self->binary_near_history);
  self->binary_near_history = NULL;

  avp_free(self->histogram);
  self->histogram = NULL;

  // BinaryDelayEstimator does not have ownership of |farend|, hence we do not
  // avp_free the memory here. That should be handled separately by the user.
  self->farend = NULL;
...
BinaryDelayEstimator* WebRtc_CreateBinaryDelayEstimator(
  ...
  if ((farend != NULL) && (max_lookahead >= 0)) {
    // Sanity conditions fulfilled.
    self = avp_malloc(sizeof(BinaryDelayEstimator));
  }
  if (self == NULL) {
    return NULL;
  }
  ...
  self->binary_near_history =
      avp_malloc((max_lookahead + 1) * sizeof(*self->binary_near_history));
...
int WebRtc_AllocateHistoryBufferMemory(BinaryDelayEstimator* self,
  ...
  self->mean_bit_counts =
      avp_realloc(self->mean_bit_counts,
              (history_size + 1) * sizeof(*self->mean_bit_counts));
  self->bit_counts =
      avp_realloc(self->bit_counts, history_size * sizeof(*self->bit_counts));
  self->histogram =
      avp_realloc(self->histogram, (history_size + 1) * sizeof(*self->histogram));
