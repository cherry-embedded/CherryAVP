...
void WebRtc_FreeDelayEstimatorFarend(void* handle) {
  ...
  avp_free(self->mean_far_spectrum);
...
void* WebRtc_CreateDelayEstimatorFarend(int spectrum_size, int history_size) {
  ...
  if (spectrum_size >= kBandLast) {
    self = avp_malloc(sizeof(DelayEstimatorFarend));
  }
  ...
  if (self != NULL) {
    ...
    self->mean_far_spectrum = avp_malloc(spectrum_size * sizeof(SpectrumType));
...
void WebRtc_FreeDelayEstimator(void* handle) {
  ...
  avp_free(self->mean_near_spectrum);
...
void* WebRtc_CreateDelayEstimator(void* farend_handle, int max_lookahead) {
  ...
  if (farend_handle != NULL) {
    self = avp_malloc(sizeof(DelayEstimator));
  }
  ...
  if (self != NULL) {
    ...
    // Allocate memory for spectrum buffers.
    self->mean_near_spectrum = avp_malloc(farend->spectrum_size *
                                      sizeof(SpectrumType));
