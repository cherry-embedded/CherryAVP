...
int32_t ALACDecoder::Init( void * inMagicCookie, uint32_t inMagicCookieSize )
{
    ...
    if (theCookieBytesRemaining >= sizeof(ALACSpecificConfig))
    {
        ...
        mMixBufferU = (int32_t *) avp_calloc( mConfig.frameLength * sizeof(int32_t), 1 );
        mMixBufferV = (int32_t *) avp_calloc( mConfig.frameLength * sizeof(int32_t), 1 );

        // allocate dynamic predictor buffer
        mPredictor = (int32_t *) avp_calloc( mConfig.frameLength * sizeof(int32_t), 1 );
