...
ALACEncoder::~ALACEncoder()
{
	...
	if ( mMixBufferU )
    {
		avp_free(mMixBufferU);
        mMixBufferU = NULL;
    }
	if ( mMixBufferV )
    {
		avp_free(mMixBufferV);
        mMixBufferV = NULL;
    }
	...
	if ( mPredictorU )
    {
		avp_free(mPredictorU);
        mPredictorU = NULL;
    }
	if ( mPredictorV )
    {
		avp_free(mPredictorV);
        mPredictorV = NULL;
    }
...
int32_t ALACEncoder::InitializeEncoder(AudioFormatDescription theOutputFormat)
{
	...
	mMixBufferU = (int32_t *) avp_calloc( mFrameSize * sizeof(int32_t), 1 );
	mMixBufferV = (int32_t *) avp_calloc( mFrameSize * sizeof(int32_t), 1 );

	// allocate dynamic predictor buffers
	mPredictorU = (int32_t *) avp_calloc( mFrameSize * sizeof(int32_t), 1 );
	mPredictorV = (int32_t *) avp_calloc( mFrameSize * sizeof(int32_t), 1 );

	// allocate combined shift buffer
	mShiftBufferUV = (uint16_t *) avp_calloc( mFrameSize * 2 * sizeof(uint16_t),1 );

	// allocate work buffer for search loop
	mWorkBuffer = (uint8_t *) avp_calloc( mMaxOutputBytes, 1 );
